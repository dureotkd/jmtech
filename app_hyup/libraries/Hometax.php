...
class hometax
{
    ...
    public function 전체자료수집($start_date, $end_date)
    {

        $this->매입세금계산서수집($start_date, $end_date);

        $this->매출세금계산서수집($start_date, $end_date);
    }

    public function 매입세금계산서수집($start_date, $end_date)
    {

        $res = $this->obj->barobill->매입세금계산서기간조회($start_date, $end_date);
        ...
        if (!empty($res)) {
            ...
            foreach ($res as $row) {
                ...
                $row = $this->obj->service_model->get_barobill_tax_invoice('row', [
                ...
                $this->obj->service_model->insert_barobill_tax_invoice(DEBUG, [
                    'type' => 'purchase',
                    'NTSSendKey' => $NTSSendKey,
                    'NTSSendDT' => $NTSSendDT,
                    'IssueDT' => $IssueDT,
    ...
    public function 매출세금계산서수집($start_date, $end_date)
    {

        $res = $this->obj->barobill->매출세금계산서조회($start_date, $end_date);
