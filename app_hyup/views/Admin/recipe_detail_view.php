...
<div class="font-sans text-sm text-gray-800">
    <div class="!mb-4 flex gap-2">
        ...
        <button class="btn" onclick="window.location.href = '/admin/recipe/create?id=<?= $recipe_row['id'] ?>'">레시피수정</button>
    </div>
    ...
    <div class="lg:flex-row lg:gap-12 gap-4 flex-col flex w-full h-full">
        ...
        <div class="lg:w-1/2 w-full max-w-[612px] z-0 !space-y-4 relative">
            <div class="w-full ">
                <img class="w-full" src="<?= $recipe_row['image_url'] ?>" alt="메인이미지" class="rounded-md w-full object-cover" />
            </div>

            <div class="">
                <!-- 난이도 / 쿠킹시간 -->
                <div class="!flex !items-center !gap-2 !text-base">
                    <span class="!font-semibold">난이도:</span>
                    <div class="rating">
                        <?
                        for ($i = 1; $i <= 5; $i++) {
                        ?>
                            <input type="radio" name="level_<?= $recipe_row['id'] ?>" <?= $recipe_row['level'] == $i ? 'checked' : '' ?> value=" <?= $i ?>" name="level" class="mask mask-star-2 bg-orange-400" aria-label="<?= $i ?> star" />
                        <?
                        }
                        ?>
                    </div>
                    <span class="!font-semibold">쿠킹시간:</span>
                    <span class="badge badge-soft !bg-gray-100 !text-gray-700 !border-gray-300 !px-3 !py-1">
                        <?= $recipe_row['cooking_time'] ?? 0 ?>분
                    </span>
                </div>
            </div>
        </div>
