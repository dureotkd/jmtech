<?
include_once $_SERVER['DOCUMENT_ROOT']."/comm/script/include_common_file.php";
include 'common.php';
...
$sql = "select * from ez_cash_receipt where id_no = {$idx} ";

$dbo->query($sql);
$뱅크정보=$dbo->next_record();
...
$공급가액 = $총결제금액 - $부가세;
    ...
    $Cashbill->identityNum = $뱅크정보['number'];

    // 구매자명
    $Cashbill->customerName = $뱅크정보['name'];

    // 주문상품명
    $Cashbill->itemName = $발행자상호.'_수수료_'.$뱅크정보['vat_amount'];

    // 주문주문번호
    $Cashbill->orderNumber = date("Ymdhi")."-".$뱅크정보['id_no'];
